...
def calculatePoints(trackWindow, radiusCircleInterest, dst):
    ...
    pts = np.array([[massCenterCol-h/2, massCenterRow-w/2],
                    [massCenterCol-h/2, massCenterRow+w/2],
                    [massCenterCol+h/2, massCenterRow+w/2],
                    [massCenterCol+h/2, massCenterRow-w/2]])
...
while True:
    ...
    if ret:
        ...
        for i in range(NUM_ITERAIONS):
            pts = calculatePoints(track_window, 10, dst)

        # Draw it on image
        # pts = cv2.boxPoints(ret)
        # pts = np.int0(pts)
        #pts = np.array([[10, 10], [800, 10], [800, 400], [10, 400]])
